---
name: watdd
description: Create a worktree, then build a feature TDD-style in it
usage: /watdd <branch-name> <feature description>
examples:
  - /watdd login add a login page that redirects to dashboard after auth
  - /watdd contact-form the contact form submits and shows a success toast
allowed-tools:
  - Bash(git:*)
  - Bash(npx:*)
  - Bash(npm:*)
...
# Worktree and TDD Skill

Create a git worktree for a new branch, then build a feature TDD-style in it — like `/tdd` but in an isolated worktree.
...
### 2. Build the Feature TDD-Style

Follow the same workflow as `/tdd` (choose test type, ensure framework, write test, confirm failure, build feature, confirm pass, slop check, update README, auto-PR check), but:
